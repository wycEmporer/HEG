...
export default function goldListion(postData){
    ...
    $.ajax({
      ...
      if(res.code === 1){
        ...
        if (self.checked) {
          ...
          if(!postData.useGold){
            let useGold = Math.min(postData.happyGoldBalance, (postData.totalPrice + postData.fee.cf));
  ...
  $(document).on('keyup blur', '#useGold', function(e) {
    ...
    useGold = Math.min.apply(null, [useGold, postData.happyGoldBalance, (postData.totalPrice + postData.fee.cf)]);
  ...
  function goldCheck(checked, useGold) {
    let lastPay = postData.totalPrice + postData.fee.cf;
    if (checked) {
      if(useGold <= 0){
