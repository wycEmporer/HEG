...
import { getBannerInfo, DUSSEHRAMEGASALE } from 'pagesDir/_component/getBannerInfo/index';
$(() => {
  ...
  // getBannerInfo(DUSSEHRAMEGASALE);

  $.ajax({
    url: path.getPromotionInfo() + 'type=13&device=1&addr=dussehra-mega-sale',
    type: 'GET',
    dataType: 'json',
  }).then(result => {
    if(result.success){
      const res= result.list[0];
      $('#banner').html(`<img src="${res.landingPageUrl == null?'':res.landingPageUrl}" alt="${res.landingPageAlt == null?'':res.landingPageAlt}" class="banImg">`);
      $('#couponPri_1').text('Rs.' + res.landingPageTitle);
      $('#couponPri_2').text('Rs.' + res.landingPageAlt);
    }
  });
  ...
  $(document).on('click', '.copyIcon', function() {
    var val = $(this).siblings('.copyText').find('.copyText_2')[0];
    var type = $(this).data('type');
    window.getSelection().selectAllChildren(val);
    document.execCommand('Copy');
    const TipsMsg_1 = $(this)
      .parents('.sale_box')
      .find('.TipsMsg_1')[0];
    const TipsMsg_2 = $(this)
      .parents('.sale_box')
      .find('.TipsMsg_2')[0];
      if(type == 1){
        $(TipsMsg_1)
...
$(() => {
  ...
  function GetRTime() {
    ...
    let timestamp_set0 = datetime_to_unix('2018-10-01 00:00:00');
    let timestamp_set2 = datetime_to_unix('2018-10-15 00:00:00');
    let timestamp_set3 = datetime_to_unix('2018-10-19 23:59:59');
    ...
    if(cha_timestamp0 <= 0){
      ...
      if( cha_timestamp2 <= 0){
        ...
        $('.Countdown .Countdown_time .end').removeClass('hidden').find('.status').text('Ends');
      ...
      $('.Countdown .Countdown_time .in').addClass('hidden');
      $('.Countdown .Countdown_time .end').removeClass('hidden').find('.status').text('Starts');
