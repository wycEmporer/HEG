...
function datepicker(dateIn, dateOut) {
  var inDate = dateIn ? new Date(dateIn) : new Date();
      ...
      for (var j = 1, lenJ = regMatch.length; j < lenJ; j++) {
          if (regMatch[j].toLowerCase() == tem.toLowerCase()) {
              if (str.toLowerCase() == 'new delhi' && 'new delhi'.indexOf(tem.toLowerCase()) == 0) {
                  replaceReturn += 'Delhi(<b style="color:#FC9525;">$' + j;
              }
              else {
...
function goToSearch(){
  ...
  if (!cityOrHotel.name && !cityOrHotel.code || !$('#cityname').val()) {
    $('#cityname').val('Bangalore');
    cityOrHotel.name = 'Bangalore';
    cityOrHotel.code = '1734';
  ...
  var param = {
    ...
    checkIn: common.format(new Date($('#in-Date').val())),
    checkOut: common.format(new Date($('#out-Date').val())),
    guests: guests
  };
