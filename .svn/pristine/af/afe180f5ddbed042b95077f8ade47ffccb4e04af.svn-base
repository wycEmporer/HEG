		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<div id="fileDragArea" class="filePicker"><p id="fileNmae">Attach Resume (Microsoft Word or PDF file is allowed (5MB))</p><p><span>Upload file</span> or drag and drop</p></div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				fileType =  filePath[filePath.length - 1]; 
					if(!(fileType == 'pdf' ||  fileType == 'docx' ||  fileType == 'doc'||  fileType == 'zip')){
						alert('Microsoft Word or PDF file is allowed (<5MB)');
					}
				}	
				// filesNum += files.length;
				var arrFiles = [];  
				console.log(filesSize);
			  if(filesSize > 5242880  || filesNum > 2){
					$('#career_Info').find('.joinReason-box .checkError').removeClass('hidden')
					.text('Microsoft Word or PDF file is allowed (<5MB)');
				}else {
					for (var i = 0, file; file = files[i]; i++) {
