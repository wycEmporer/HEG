...
import { getBannerInfo, PRECHRISTMASSALE } from 'pagesDir/_component/getBannerInfo/index';
...
$(() => {
  ...
  getBannerInfo(PRECHRISTMASSALE);
  if(!isLogin){
  ...
  // copy code
  $(document).on('click', '.copyText_2', function() {
    var val = $(this)[0];
  ...
  $(document).on('click', '.copyText_1', function() {
    var val = $(this)[0];
    var type = $(this).data('type');
    window.getSelection().selectAllChildren(val);
    document.execCommand('Copy');
    const TipsMsg = $(this).parents('p')
      .siblings('.TipsMsg')[0];
...
$(() => {
  ...
  function GetRTime() {
    ...
    let timestamp_set0 = datetime_to_unix('2018-11-13 00:00:00');
    let timestamp_set2 = datetime_to_unix('2018-11-21 00:00:00');
    let timestamp_set3 = datetime_to_unix('2018-12-25 23:59:59');
