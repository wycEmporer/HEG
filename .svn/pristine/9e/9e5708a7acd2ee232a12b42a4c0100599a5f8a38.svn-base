...
(function($) {
  $.extend({
    ...
    /**
     * format date
     * @param  {str} strDate 2017/2/22
     * @return {str}         week, 22 Feb 2017
     */
    formatStr: function(strDate) {
      if (typeof strDate != 'string') return new Date();
      var longTime = Date.parse(strDate.replace(/-/g, '/'));
      if (isNaN(longTime)) {
        console.log('strDate formate is wrong');
        return new Date();
      }
      var week = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
      var month = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
      var dateObj = new Date(longTime);
      var newStr = week[dateObj.getDay()] + ', ' + (dateObj.getDate().toString().length == 1 ? '0' + dateObj.getDate() : dateObj.getDate()) + ' ' + month[dateObj.getMonth()] + ' ' + dateObj.getFullYear();
      return newStr;
    },
    // formatDetailStr: function()
    /**
     * 将字符串解析成日期
    ...
    formatDurationToFriendly: function(second) {
      ...
      // else if(second < 60*60) return (second-second%60)/60+'m'+' '+second%60+'s';
      else if (second < 60 * 60) return (second - (second % 60)) / 60 + 'm';
      else if (second < 60 * 60 * 24) return (second - (second % 3600)) / 60 / 60 + 'h' + ' ' + Math.round((second % 3600) / 60) + 'm';
      return (second - (second % (3600 * 24))) / 3600 / 24 + 'd' + Math.round((second % (3600 * 24)) / 60 / 60) + 'h';
    },
    /**
     *将两个字符串日期,求时间差;
     * parma1 str ,parma2 str
     * 输出友好格式时间差
     */
    countTime: function(str1, str2) {
      var diff = Date.parse(str2) - Date.parse(str1);
      return this.formatDurationToFriendly(diff / 1000);
    },
    /**
     * 将时间转换成MM:SS形式
     */
