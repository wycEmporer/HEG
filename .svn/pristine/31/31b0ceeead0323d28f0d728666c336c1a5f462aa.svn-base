	...
	$.fn.zyUpload = function(options, param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '	<div class="upload_box clearfix">';
					html += '		<div class="upload_main">';
					html += '			<div class="upload_choose clearfix">';
	            	html += '				<div class="convent_choice">';
	            	html += '					<div class="andArea">';
	            	html += '						<div id="fileDragArea" class="filePicker"><p id="fileNmae">Attach File (IMG file is allowed (500KB))</p><p><a class="a_link">Upload file</a> or drag and drop</p></div>';
	            	html += '						<input id="fileImage" accept="pdf/docx" type="file" size="30" name="file" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					...
					file_Type =  filePath[filePath.length - 1]; 
					if(para.fileType.indexOf(file_Type) == -1 ){
						$('#fileNmae').text('This file is not supported.').addClass('colorFail');
						continue;
					}
					console.log(file.size);
					if (file.size >= 512000) {
						$('#fileNmae').text('IMG file is allowed (500KB)').addClass('colorFail');
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
						$('#fileNmae').text(file.name).removeClass('colorFail');
					}
				}
				return arrFiles;
			};
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						// $('#fileNmae').text(selectFiles[0].name);
						var html = '', i = 0;
						// 组织预览html
